...
def build(build_type="Release"):
    print(f"Building ({build_type})...", file=sys.stderr)
    target = "all" if build_type == "Release" else "debug"
...
def main():
    ...
    ap.add_argument("--skip-build", action="store_true", help="reuse existing ./navsolver")
    ...
    rows = []
    print(f"{'size':<8} {'grid':<14} {'cells':>10} {'min s':>10} {'s/step':>10} {'ns/cell-step':>14}")
    with tempfile.TemporaryDirectory() as tmp:
        for size in sizes:
            nx, ny, nz = SIZE_MATRIX[size]
            out_dir = Path(tmp) / size
            cfg_path = Path(tmp) / f"{size}.cfg"
            ...
            for _ in range(args.repeats):
                elapsed, active_cells, steps_ran = run_once(cfg_path)
                times.append(elapsed)

            if steps_ran != args.steps:
                print(f"  WARNING: {size} ran {steps_ran} steps, expected {args.steps} "
            ...
            print(f"{size:<8} {f'{nx}x{ny}x{nz}':<14} {active_cells or 0:>10} "
